.class public final Lw7/s;
.super Lw7/u;
.source "SourceFile"


# instance fields
.field public final U:LM/f;

.field public final V:LM/f;

.field public W:J


# direct methods
.method public constructor <init>(Lw7/b0;)V
    .locals 1

    invoke-direct {p0, p1}, LK0/p;-><init>(Lw7/b0;)V

    new-instance p1, LM/f;

    const/4 v0, 0x0

    invoke-direct {p1, v0}, LM/y;-><init>(I)V

    iput-object p1, p0, Lw7/s;->V:LM/f;

    new-instance p1, LM/f;

    invoke-direct {p1, v0}, LM/y;-><init>(I)V

    iput-object p1, p0, Lw7/s;->U:LM/f;

    return-void
.end method


# virtual methods
.method public final T1(Ljava/lang/String;J)V
    .locals 8

    iget-object v0, p0, LK0/p;->T:Ljava/lang/Object;

    check-cast v0, Lw7/b0;

    if-eqz p1, :cond_1

    invoke-virtual {p1}, Ljava/lang/String;->length()I

    move-result v1

    if-nez v1, :cond_0

    goto :goto_0

    :cond_0
    iget-object v0, v0, Lw7/b0;->c0:Lw7/a0;

    invoke-static {v0}, Lw7/b0;->f(Lw7/f0;)V

    new-instance v7, Lw7/a;

    const/4 v6, 0x0

    move-object v1, v7

    move-object v2, p0

    move-object v3, p1

    move-wide v4, p2

    invoke-direct/range {v1 .. v6}, Lw7/a;-><init>(Lw7/s;Ljava/lang/String;JI)V

    invoke-virtual {v0, v7}, Lw7/a0;->a2(Ljava/lang/Runnable;)V

    return-void

    :cond_1
    :goto_0
    iget-object p0, v0, Lw7/b0;->b0:Lw7/H;

    invoke-static {p0}, Lw7/b0;->f(Lw7/f0;)V

    const-string p1, "Ad unit id must be a non-empty string"

    iget-object p0, p0, Lw7/H;->Y:Lw7/F;

    invoke-virtual {p0, p1}, Lw7/F;->a(Ljava/lang/String;)V

    return-void
.end method

.method public final U1(Ljava/lang/String;J)V
    .locals 8

    iget-object v0, p0, LK0/p;->T:Ljava/lang/Object;

    check-cast v0, Lw7/b0;

    if-eqz p1, :cond_1

    invoke-virtual {p1}, Ljava/lang/String;->length()I

    move-result v1

    if-nez v1, :cond_0

    goto :goto_0

    :cond_0
    iget-object v0, v0, Lw7/b0;->c0:Lw7/a0;

    invoke-static {v0}, Lw7/b0;->f(Lw7/f0;)V

    new-instance v7, Lw7/a;

    const/4 v6, 0x1

    move-object v1, v7

    move-object v2, p0

    move-object v3, p1

    move-wide v4, p2

    invoke-direct/range {v1 .. v6}, Lw7/a;-><init>(Lw7/s;Ljava/lang/String;JI)V

    invoke-virtual {v0, v7}, Lw7/a0;->a2(Ljava/lang/Runnable;)V

    return-void

    :cond_1
    :goto_0
    iget-object p0, v0, Lw7/b0;->b0:Lw7/H;

    invoke-static {p0}, Lw7/b0;->f(Lw7/f0;)V

    const-string p1, "Ad unit id must be a non-empty string"

    iget-object p0, p0, Lw7/H;->Y:Lw7/F;

    invoke-virtual {p0, p1}, Lw7/F;->a(Ljava/lang/String;)V

    return-void
.end method

.method public final V1(J)V
    .locals 6

    iget-object v0, p0, LK0/p;->T:Ljava/lang/Object;

    check-cast v0, Lw7/b0;

    iget-object v0, v0, Lw7/b0;->h0:Lw7/E0;

    invoke-static {v0}, Lw7/b0;->e(Lw7/L;)V

    const/4 v1, 0x0

    invoke-virtual {v0, v1}, Lw7/E0;->Y1(Z)Lw7/C0;

    move-result-object v0

    iget-object v1, p0, Lw7/s;->U:LM/f;

    invoke-virtual {v1}, LM/f;->keySet()Ljava/util/Set;

    move-result-object v2

    check-cast v2, LM/c;

    invoke-virtual {v2}, LM/c;->iterator()Ljava/util/Iterator;

    move-result-object v2

    :goto_0
    invoke-interface {v2}, Ljava/util/Iterator;->hasNext()Z

    move-result v3

    if-eqz v3, :cond_0

    invoke-interface {v2}, Ljava/util/Iterator;->next()Ljava/lang/Object;

    move-result-object v3

    check-cast v3, Ljava/lang/String;

    invoke-virtual {v1, v3}, LM/y;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v4

    check-cast v4, Ljava/lang/Long;

    invoke-virtual {v4}, Ljava/lang/Long;->longValue()J

    move-result-wide v4

    sub-long v4, p1, v4

    invoke-virtual {p0, v3, v4, v5, v0}, Lw7/s;->X1(Ljava/lang/String;JLw7/C0;)V

    goto :goto_0

    :cond_0
    invoke-virtual {v1}, LM/y;->isEmpty()Z

    move-result v1

    if-nez v1, :cond_1

    iget-wide v1, p0, Lw7/s;->W:J

    sub-long v1, p1, v1

    invoke-virtual {p0, v1, v2, v0}, Lw7/s;->W1(JLw7/C0;)V

    :cond_1
    invoke-virtual {p0, p1, p2}, Lw7/s;->Y1(J)V

    return-void
.end method

.method public final W1(JLw7/C0;)V
    .locals 2

    iget-object p0, p0, LK0/p;->T:Ljava/lang/Object;

    check-cast p0, Lw7/b0;

    if-nez p3, :cond_0

    iget-object p0, p0, Lw7/b0;->b0:Lw7/H;

    invoke-static {p0}, Lw7/b0;->f(Lw7/f0;)V

    const-string p1, "Not logging ad exposure. No active activity"

    iget-object p0, p0, Lw7/H;->g0:Lw7/F;

    invoke-virtual {p0, p1}, Lw7/F;->a(Ljava/lang/String;)V

    return-void

    :cond_0
    const-wide/16 v0, 0x3e8

    cmp-long v0, p1, v0

    if-gez v0, :cond_1

    iget-object p0, p0, Lw7/b0;->b0:Lw7/H;

    invoke-static {p0}, Lw7/b0;->f(Lw7/f0;)V

    invoke-static {p1, p2}, Ljava/lang/Long;->valueOf(J)Ljava/lang/Long;

    move-result-object p1

    iget-object p0, p0, Lw7/H;->g0:Lw7/F;

    const-string p2, "Not logging ad exposure. Less than 1000 ms. exposure"

    invoke-virtual {p0, p1, p2}, Lw7/F;->b(Ljava/lang/Object;Ljava/lang/String;)V

    return-void

    :cond_1
    new-instance v0, Landroid/os/Bundle;

    invoke-direct {v0}, Landroid/os/Bundle;-><init>()V

    const-string v1, "_xt"

    invoke-virtual {v0, v1, p1, p2}, Landroid/os/BaseBundle;->putLong(Ljava/lang/String;J)V

    const/4 p1, 0x1

    invoke-static {p3, v0, p1}, Lw7/e1;->e2(Lw7/C0;Landroid/os/Bundle;Z)V

    iget-object p0, p0, Lw7/b0;->i0:Lw7/y0;

    invoke-static {p0}, Lw7/b0;->e(Lw7/L;)V

    const-string p1, "am"

    const-string p2, "_xa"

    invoke-virtual {p0, p1, p2, v0}, Lw7/y0;->Z1(Ljava/lang/String;Ljava/lang/String;Landroid/os/Bundle;)V

    return-void
.end method

.method public final X1(Ljava/lang/String;JLw7/C0;)V
    .locals 2

    iget-object p0, p0, LK0/p;->T:Ljava/lang/Object;

    check-cast p0, Lw7/b0;

    if-nez p4, :cond_0

    iget-object p0, p0, Lw7/b0;->b0:Lw7/H;

    invoke-static {p0}, Lw7/b0;->f(Lw7/f0;)V

    const-string p1, "Not logging ad unit exposure. No active activity"

    iget-object p0, p0, Lw7/H;->g0:Lw7/F;

    invoke-virtual {p0, p1}, Lw7/F;->a(Ljava/lang/String;)V

    return-void

    :cond_0
    const-wide/16 v0, 0x3e8

    cmp-long v0, p2, v0

    if-gez v0, :cond_1

    iget-object p0, p0, Lw7/b0;->b0:Lw7/H;

    invoke-static {p0}, Lw7/b0;->f(Lw7/f0;)V

    invoke-static {p2, p3}, Ljava/lang/Long;->valueOf(J)Ljava/lang/Long;

    move-result-object p1

    iget-object p0, p0, Lw7/H;->g0:Lw7/F;

    const-string p2, "Not logging ad unit exposure. Less than 1000 ms. exposure"

    invoke-virtual {p0, p1, p2}, Lw7/F;->b(Ljava/lang/Object;Ljava/lang/String;)V

    return-void

    :cond_1
    new-instance v0, Landroid/os/Bundle;

    invoke-direct {v0}, Landroid/os/Bundle;-><init>()V

    const-string v1, "_ai"

    invoke-virtual {v0, v1, p1}, Landroid/os/BaseBundle;->putString(Ljava/lang/String;Ljava/lang/String;)V

    const-string p1, "_xt"

    invoke-virtual {v0, p1, p2, p3}, Landroid/os/BaseBundle;->putLong(Ljava/lang/String;J)V

    const/4 p1, 0x1

    invoke-static {p4, v0, p1}, Lw7/e1;->e2(Lw7/C0;Landroid/os/Bundle;Z)V

    iget-object p0, p0, Lw7/b0;->i0:Lw7/y0;

    invoke-static {p0}, Lw7/b0;->e(Lw7/L;)V

    const-string p1, "am"

    const-string p2, "_xu"

    invoke-virtual {p0, p1, p2, v0}, Lw7/y0;->Z1(Ljava/lang/String;Ljava/lang/String;Landroid/os/Bundle;)V

    return-void
.end method

.method public final Y1(J)V
    .locals 4

    iget-object v0, p0, Lw7/s;->U:LM/f;

    invoke-virtual {v0}, LM/f;->keySet()Ljava/util/Set;

    move-result-object v1

    check-cast v1, LM/c;

    invoke-virtual {v1}, LM/c;->iterator()Ljava/util/Iterator;

    move-result-object v1

    :goto_0
    invoke-interface {v1}, Ljava/util/Iterator;->hasNext()Z

    move-result v2

    if-eqz v2, :cond_0

    invoke-interface {v1}, Ljava/util/Iterator;->next()Ljava/lang/Object;

    move-result-object v2

    check-cast v2, Ljava/lang/String;

    invoke-static {p1, p2}, Ljava/lang/Long;->valueOf(J)Ljava/lang/Long;

    move-result-object v3

    invoke-virtual {v0, v2, v3}, LM/y;->put(Ljava/lang/Object;Ljava/lang/Object;)Ljava/lang/Object;

    goto :goto_0

    :cond_0
    invoke-virtual {v0}, LM/y;->isEmpty()Z

    move-result v0

    if-nez v0, :cond_1

    iput-wide p1, p0, Lw7/s;->W:J

    :cond_1
    return-void
.end method
